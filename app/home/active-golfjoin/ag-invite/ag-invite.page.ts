...
@Component({
  selector: 'app-ag-invite',
  templateUrl: './ag-invite.page.html',
  styleUrls: ['./ag-invite.page.scss'],
})
export class AgInvitePage implements OnInit {
  ...
  ngOnInit() {
    this.auth.fbScreen('golfjoin_invite_list');
    this.getLists();
  }

  getLists() {
    this.golfjoinS.getLists({ type: 'invite' }, this.limit, this.offset)
    .subscribe(res => {
      if (res.success) {
