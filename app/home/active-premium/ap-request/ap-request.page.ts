...
@Component({
  ...
  templateUrl: './ap-request.page.html',
  styleUrls: ['./ap-request.page.scss'],
})
export class ApRequestPage implements OnInit {
  ...
  ngOnInit() {
    this.auth.fbScreen('premium_request_list');
    this.getLists();
  }

  getLists() {
    this.premiumS.getRequest({}, this.limit, this.offset)
    .subscribe(res => {
      if (res.success) {
