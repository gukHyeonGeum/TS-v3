...
@Component({
  ...
  styleUrls: ['./updated.page.scss'],
})
export class UpdatedPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.auth.fbScreen('member_update_list');
  }

  getLists() {
    this.memberS.getLists({ type: 'updated'}, this.limit, this.offset)
    .subscribe(res => {
      if (res.success) {
